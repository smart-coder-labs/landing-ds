...
import { DiffViewerProps, DiffViewMode } from './DiffViewer.types';
    ...
    const renderSplit = () => {
        ...
        diffLines.forEach(line => {
            if (line.type === 'unchanged') {
                ...
                leftSide.push(line);
                rightSide.push({ type: 'unchanged', content: '', newLineNumber: undefined });
            } else if (line.type === 'added') {
                leftSide.push({ type: 'unchanged', content: '', oldLineNumber: undefined });
                rightSide.push(line);
            }
        });
                    ...
                    {leftSide.map((line, idx) => (
                        <div 
                            ...
                            className={cn(
                                "flex",
                                line.type === 'removed' && "bg-red-100/50 dark:bg-red-900/20",
                                !line.oldLineNumber && line.content === '' && "bg-black/5 dark:bg-white/5"
                            )}
                        >
                            <div className="flex-none w-12 text-right pr-4 py-0.5 select-none text-text-tertiary border-r border-border-primary/30 opacity-50">
                                {line.oldLineNumber || ''}
